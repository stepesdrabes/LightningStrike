...
/** Require independent model, full-mix attack and isolated low-frequency evidence. */
export function mergeKickEvidence(legacy: Stream, dsp: DrumStream, source: Float32Array,
	mix: Float32Array, model: DrumStream, rate: number) {
	...
	return { times: hits.map(e => e.time), levels: hits.map(e => e.level), invented: hits.map(e => e.invented) };
}
