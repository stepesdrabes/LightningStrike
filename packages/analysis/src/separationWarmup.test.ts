	...
	openSession: async (path: string, options: { executionProviders: string[] }) => {
		...
		return {
			...
			run: async (feeds: Record<string, { data: Float32Array; dims: number[] }>) => {
				const [input, magnitude] = [feeds.mix ?? feeds.waveform, feeds.magnitude];
				...
				const sources = feeds.mix ? 1 : 4;
				const wave = new Float32Array(sources * 2 * input.dims[2]);
				for (let s = 0; s < sources; s++) wave.set(input.data, s * input.data.length);
				const time = calls.silentDrums && feeds.mix ? new Float32Array(wave.length) : wave;
				const frequency = new Float32Array(sources * magnitude.data.length);
				return {
					time_output: { data: time, dims: [1, sources, 2, input.dims[2]] },
					freq_output: { data: frequency, dims: [1, sources, ...magnitude.dims.slice(1)] }
				};
			}
		};
	}
...
describe('DirectML kit warm-up', () => {
	it('compiles DrumSep beside HTDemucs and reuses that session for the kit chunks', async () => {
		const result = await (separator('dml') as any).separate([input, input]);
		expect(result.kick).toHaveLength(input.length);
		expect(calls.opened).toEqual([
			{ model: 'drumsep.onnx', provider: 'dml' },
			{ model: 'htdemucs.onnx', provider: 'dml' }
		]);
		expect(calls.runs).toEqual(['drumsep.onnx:dml:0:warmup', 'htdemucs.onnx:dml:1', 'drumsep.onnx:dml:0']);
		expect([...calls.released].sort()).toEqual(['drumsep.onnx:dml:0', 'htdemucs.onnx:dml:1']);
	});
	...
	it('restarts the kit stage on CPU when the warm-up fails', async () => {
		...
		expect(calls.opened.at(-1)).toEqual({ model: 'drumsep.onnx', provider: 'cpu' });
	...
	it('releases an unused warm session when the track has no drums', async () => {
		...
		expect(calls.runs).toEqual(['drumsep.onnx:dml:0:warmup', 'htdemucs.onnx:dml:1']);
		expect([...calls.released].sort()).toEqual(['drumsep.onnx:dml:0', 'htdemucs.onnx:dml:1']);
	});
...
describe('DirectML HTDemucs compilation during model verification', () => {
	...
	async function verifying() {
		...
		for (const name of ['htdemucs.onnx', 'drumsep.onnx']) writeFileSync(join(directory, name), '');
	...
	it('opens HTDemucs before the checksums finish and separates with that session', async () => {
		...
		expect(calls.runs).toEqual(['drumsep.onnx:dml:1:warmup', 'htdemucs.onnx:dml:0', 'drumsep.onnx:dml:1']);
		expect([...calls.released].sort()).toEqual(['drumsep.onnx:dml:1', 'htdemucs.onnx:dml:0']);
	});
